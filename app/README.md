...
- **`manage.sh`** — `stop` / `start` / `status` for the running stack. `stop` scales the Auto Scaling Group to 0 and stops the data instance (NAT + ALB stay up, nothing deleted); `start` brings it all back and waits for healthy targets.
