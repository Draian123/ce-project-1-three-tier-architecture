...
def db_up():
    try:
        s=socket.create_connection((DB_HOST,DB_PORT),timeout=1); s.close(); return True
    except Exception:
        return False
class H(http.server.BaseHTTPRequestHandler):
    def log_message(self,*a): pass
    def do_GET(self):
        iid=meta("instance-id"); az=meta("placement/availability-zone"); up=db_up()
        if self.path.rstrip("/")=="/health":
            body=json.dumps({"status":"healthy","instance":iid,"az":az,
                "db":"up" if up else "down","db_host":DB_HOST}).encode()
            self.send_response(200); self.send_header("Content-Type","application/json")
            self.send_header("Content-Length",str(len(body))); self.end_headers(); self.wfile.write(body); return
        color="#12805c" if up else "#b42318"; label="Connected" if up else "Unreachable"
...
.card{{background:#1e293b;border:1px solid #334155;border-radius:16px;padding:34px 40px;max-width:440px;box-shadow:0 12px 40px rgba(0,0,0,.4)}}
...
<div class=row><span class=k>Data tier ({DB_HOST}:{DB_PORT})</span><span class="v pill"><span class=dot></span>{label}</span></div>
